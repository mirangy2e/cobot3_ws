...
class FrankaTask(BaseTask):
    ...
    def _setup_physics(self):
        ...
        # 그리퍼 Drive (별도 값)
        # for jpath in gripper_joints:
        #     jprim = stage.GetPrimAtPath(jpath)
        #     if jprim.IsValid():
        #         d = UsdPhysics.DriveAPI.Get(jprim, "linear")
        #         if not d:
        #             d = UsdPhysics.DriveAPI.Apply(jprim, "linear")
        #         d.GetStiffnessAttr().Set(1e3)
        #         d.GetDampingAttr().Set(1e1)
        #         # d.GetMaxForceAttr().Set(1e1)
        # print("  [OK] 그리퍼 Drive  stiffness=1e2  damping=1e1  maxforce=1e2")
