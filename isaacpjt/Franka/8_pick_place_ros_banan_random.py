...
def main():
    ...
    while simulation_app.is_running():
        ...
        if is_playing and not was_playing:
            ...
            # 바나나 Y축 랜덤 스폰 (-0.1 ~ 0.1)
            banana = my_world.scene.get_object("banana")
            pos, ori = banana.get_world_pose()
            pos[1] = np.random.uniform(-0.1, 0.1)
            banana.set_world_pose(position=pos, orientation=ori)
            print(f"[스폰] 바나나 Y={pos[1]:.3f}m")
            print("[대기] /banana/point_world 수신 대기 중...")
        ...
        if is_playing and not task_done:
            ...
            if not pick_started:
                ...
                if pose is not None:
                    ...
                    cube_pos_from_topic[2] = 0.02   # Z 보정 (depth는 표면, pick은 바닥 기준)
