# -*- coding: utf-8 -*-
"""
Franka 바나나 Pick & Place  —  RMPflow + FSM + ROS2 Camera Graph
=================================================================
GUI의 Tools > Robotics > ROS 2 OmniGraphs > Camera 로 생성한 그래프를
graph_dump.txt 에서 확인한 노드/설정값 그대로 코드로 재현.

ROS2 Camera Graph  (/World/Graph/camera_graph)
  노드 구성 (graph_dump.txt 기준)
    OnPlaybackTick  — 매 플레이백 틱마다 퍼블리셔 실행
    RunOnce         — 시뮬레이션 첫 프레임에 RenderProduct 1회 생성
    Context         — ROS2 컨텍스트 (domain_id=50, useDomainIDEnvVar=True)
    RenderProduct   — IsaacCreateRenderProduct  (640×640, cameraPrim=/World/Camera)
    RGBPublish      — ROS2CameraHelper          → /camera/rgb
    DepthPublish    — ROS2CameraHelper          → /camera/depth
    CameraInfoPublish — ROS2CameraInfoHelper    → camera_info
...
실행
  ~/isaacsim/python.sh ~/cobot3_ws/isaacpjt/practice/step07a_ros2_actiongraph_camera.py
...
# ── ROS2 Camera Graph ─────────────────────────────────────────────
# graph_dump.txt 에서 확인한 실제 노드명·설정값 사용
...
# ================================================================
# ROS2 Camera Graph  — 워밍업 이후 생성  (graph_dump.txt 기준)
# ================================================================
# ※ 이 블록은 PhysX 워밍업(render=False) 이후에 위치해야 함.
...
def _build_camera_graph():
    """
    graph_dump.txt 에서 확인한 노드명·설정값으로 Camera Graph 생성.

    exec 체인 (graph_dump.txt CONNECT 기준):
      OnPlaybackTick.outputs:tick → RunOnce.inputs:execIn
      RunOnce.outputs:step        → RenderProduct.inputs:execIn      (첫 프레임 1회)
      RenderProduct.outputs:execOut → RGBPublish.inputs:execIn       (매 프레임)
      RenderProduct.outputs:execOut → DepthPublish.inputs:execIn
    ...
    print(f"✅ ROS2 Camera Graph 생성 완료  →  {CAM_GRAPH_PATH}")
    print(f"   domain_id   : {CAM_DOMAIN_ID}  (useDomainIDEnvVar=True)")
...
# 워밍업 완료 후 Camera Graph 생성
# → RunOnce가 render=True 첫 프레임에서 발화하도록 타이밍 보장
try:
    _build_camera_graph()
except Exception as e:
    print(f"⚠  Camera Graph 생성 실패: {e}")
    traceback.print_exc()
...
print("\n" + "=" * 60)
print("  ▶  Franka 바나나 Pick & Place  +  ROS2 Camera")
