...
simulation_app = SimulationApp({"headless": True})   # 보면서 확인하려면 False
...
# 미사용 (seg 실루엣에 나타나지 않거나 정상과 구분 안 되는 결함)
# SCRATCH_USD  = f"{BASE}/can_demaged_scratch.usdc"   # 색 결함 -> 마스크 외곽선에 안 나타남
# LIGHT_USD    = f"{BASE}/can_damage_light.usdc"
# DECOBOKO_USD = f"{BASE}/can_demaged_decoboko.usdc"

# [검증] 정상 + severe 2개만. top-view에서 dent가 외곽선에 드러나는지 확인용.
CANS = [
    ("/World/can_normal",  NORMAL_USD,  "ok"),
    ("/World/can_severe",  SEVERE_USD,  "ng"),   # 림·뚜껑 변형
]

# 2개 슬롯. 중앙에 가까울수록 원근이 줄어 옆면이 덜 보임 -> 위쪽 실루엣만 남음
SLOT_CENTERS = [(-0.055, 0.0), (0.055, 0.0)]
JITTER   = 0.005
CAN_Z    = 0.0
GROUND_Z = -0.045
...
CAM_H      = 1.2         # [검증] 멀리 + 망원 -> 원근 축소(옆면 덜 보임). 기본 0.45
CAM_FOCAL  = 65.0        # [검증] 망원. 거리에 비례해 늘려 캔 크기 유지. 기본 24.0
CAM_CLIP   = (0.01, 1000.0)
RESOLUTION = (1280, 1280)   # YOLO용: 정사각형(letterbox 방지) + 결함 디테일 여유
CAM_TARGET   = (0.0, 0.0, 0.0)   # 카메라가 바라보는 중심(캔 영역)
TILT_MAX_DEG = 0.0               # [검증] 완전 수직 top-view 고정 (원복 시 25.0)
...
def main():
    ...
    writer.initialize(output_dir=OUT_DIR, rgb=True,
                      bounding_box_2d_tight=True,      # 기존 검출용(유지)
                      instance_segmentation=True)      # seg 학습용 마스크
    writer.attach([rp])
    update(15)   # 워밍업
